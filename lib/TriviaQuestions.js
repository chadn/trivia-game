...
TriviaQuestions.prototype.generateMultiplyQuestionObj = function() {
    ...
    var a;
    var unitsDigit = this.correctAnswer - 10*Math.floor(this.correctAnswer/10);
    for (;this.qo.choices.length < 5;) {
        
        // make sure generated number has same units digit as correct answer (harder to cheat)
        a = Math.floor(randomNumber({ pct:50, num:this.correctAnswer })/10)*10 + unitsDigit;
